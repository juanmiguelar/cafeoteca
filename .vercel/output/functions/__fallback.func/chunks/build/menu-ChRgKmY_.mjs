...
import { e as ssrRenderAttrs_1, a as ssrRenderComponent_1, f as ssrRenderList_1, g as ssrRenderClass_1, i as ssrInterpolate_1 } from '../routes/renderer.mjs';
import './server.mjs';
...
const _sfc_main = /* @__PURE__ */ vueExports.defineComponent({
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      ssrRenderList_1(vueExports.unref(menuCategories), (category) => {
        ...
        ])}">${ssrInterpolate_1(category.label)}</button>`);
      });
      _push(`<!--]--></div></nav><div class="px-4 sm:px-6 lg:px-8 py-8 space-y-12 lg:space-y-16"><!--[-->`);
      ssrRenderList_1(vueExports.unref(filteredSections), (section) => {
        _push(`<section><!--[-->`);
        ssrRenderList_1(section.subsections, (sub, subIdx) => {
          _push(`<div class="${ssrRenderClass_1(["mb-10 lg:mb-14", subIdx > 0 ? "mt-8" : ""])}"><div class="flex items-center gap-3 mb-6"><div class="${ssrRenderClass_1(["w-1 h-8 rounded-full", sectionColorBg(section.color)])}"></div><h2 class="font-display font-bold text-xl sm:text-2xl lg:text-3xl text-gray-900">${ssrInterpolate_1(sub.title)}</h2></div>`);
          if (sub.note) {
            _push(`<p class="text-gray-500 text-sm sm:text-base mb-6 max-w-2xl leading-relaxed italic">${ssrInterpolate_1(sub.note)}</p>`);
          } else {
            _push(`<!---->`);
          }
          if (sub.items.length === 0) {
            ...
            ssrRenderList_1(sub.items, (item) => {
              _push(`<article class="bg-white rounded-xl p-4 sm:p-5 shadow-sm border border-gray-100 hover:shadow-md transition-shadow"><div class="flex justify-between items-start gap-3 mb-1"><h3 class="font-bold text-gray-900 text-sm sm:text-base leading-snug">${ssrInterpolate_1(item.name)}</h3>`);
              if (item.price) {
                _push(`<span class="${ssrRenderClass_1(["font-serif font-bold text-sm sm:text-base whitespace-nowrap", sectionColorText(section.color)])}">${ssrInterpolate_1(item.price)}</span>`);
              } else {
                _push(`<!---->`);
              }
              _push(`</div>`);
              if (item.description) {
                _push(`<p class="text-gray-500 text-xs sm:text-sm leading-relaxed mt-1">${ssrInterpolate_1(item.description)}</p>`);
              } else {
                _push(`<!---->`);
              }
              if (item.variants && item.variants.length) {
                _push(`<div class="mt-2 space-y-1"><!--[-->`);
                ssrRenderList_1(item.variants, (variant) => {
                  _push(`<div class="flex justify-between text-xs sm:text-sm"><span class="text-gray-600">${ssrInterpolate_1(variant.name)}</span><span class="${ssrRenderClass_1(["font-semibold", sectionColorText(section.color)])}">${ssrInterpolate_1(variant.price)}</span></div>`);
                });
                _push(`<!--]--></div>`);
              } else {
                _push(`<!---->`);
              ...
              if (item.options && item.options.length) {
                ...
                ssrRenderList_1(item.options, (opt) => {
                  _push(`<span class="text-[10px] sm:text-xs bg-gray-100 text-gray-600 px-2 py-0.5 rounded-full">${ssrInterpolate_1(opt)}</span>`);
                });
                _push(`<!--]--></div>`);
              } else {
                _push(`<!---->`);
