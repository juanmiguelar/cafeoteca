...
const _routes = [
  {
    name: "menu",
    path: "/menu",
    component: () => import('./menu-ChRgKmY_.mjs')
  },
  {
    name: "index",
    path: "/",
    component: () => import('./index-Y2C_wX--.mjs')
  },
  {
    name: "nosotros",
    path: "/nosotros",
    component: () => import('./nosotros-C8e-tJtA.mjs')
  }
];
...
function createSitePathResolver(options = {}) {
  ...
  return (path) => {
    return vueExports.computed(() => resolveSitePath(vueExports.unref(path), {
      absolute: vueExports.unref(options.absolute),
      withBase: vueExports.unref(options.withBase),
      siteUrl: vueExports.unref(options.canonical) !== false || false ? siteConfig.url : nitroOrigin,
...
function initPlugin(nuxtApp) {
  ...
  const schemaOrg = vueExports.computed(() => {
    const siteConfigResolved = {};
    for (const key in siteConfig) {
      if (key.startsWith("_")) {
        continue;
      }
      siteConfigResolved[key] = vueExports.toValue(siteConfig[key]);
      if (typeof siteConfigResolved[key] === "object") {
        for (const k in siteConfigResolved[key]) {
          siteConfigResolved[key][k] = vueExports.toValue(siteConfigResolved[key][k]);
        }
      }
    }
    return {
      ...
      ...siteConfigResolved,
      url: vueExports.toValue(resolveUrl(route.path)),
      host: withTrailingSlash(siteConfigResolved.url),
      inLanguage: vueExports.toValue(siteConfigResolved.currentLocale) || vueExports.toValue(siteConfigResolved.defaultLocale),
      path: vueExports.toValue(resolvePath(route.path))
    };
  });
  const templateParamEntry = useHead({
    templateParams: { schemaOrg: schemaOrg.value }
  });
  vueExports.watch(() => siteConfig, () => {
...
function maybeAddIdentitySchemaOrg() {
  ...
  if (config.identity || siteConfig.identity) {
    ...
    let identityPayload = {
      name: () => vueExports.toValue(siteConfig.name),
      url: () => vueExports.toValue(siteConfig.url)
    };
    let identityType;
...
const defaults_ZjgoYqsIrjWNaJMfDhci2B0eoNnvY4CDsoscm0L1fE0 = /* @__PURE__ */ defineNuxtPlugin({
  ...
  setup() {
    ...
    useSchemaOrg([
      defineWebSite({
        name: () => vueExports.toValue(siteConfig.name) || "",
        inLanguage: () => vueExports.toValue(siteConfig.currentLocale) || "",
        description: () => vueExports.toValue(siteConfig.description) || ""
      }),
      defineWebPage()
    ]);
...
const material_icons_EyklNBrlnISJ1cIz5NtE7qYZh3MTQi_7AvCCDJIkMg4 = /* @__PURE__ */ defineNuxtPlugin(() => {
  useHead({
    link: [
      {
        rel: "stylesheet",
        ...
        "tags:resolve": async ({ tags }) => {
          for (const tag of tags) {
            ...
            if (typeof tag.props.content !== "string" || !tag.props.content.trim() || tag.props.content.startsWith("http") || tag.props.content.startsWith("//"))
              continue;
            tag.props.content = vueExports.unref(resolver(tag.props.content));
          }
        }
      }
    });
...
function applyDefaults() {
  ...
  const resolveCurrentLocale = () => {
    const locale = vueExports.toValue(siteConfig.currentLocale) || vueExports.toValue(siteConfig.defaultLocale) || "en";
  ...
  const err = /* @__PURE__ */ useError();
  const canonicalUrl = vueExports.computed(() => {
    if (err.value) {
      return false;
    }
    const { query } = route;
...
const plugins = [
  ...
  material_icons_EyklNBrlnISJ1cIz5NtE7qYZh3MTQi_7AvCCDJIkMg4,
...
const layouts = {
  default: vueExports.defineAsyncComponent(() => import('./default-d1FVjaRj.mjs').then((m) => m.default || m))
};
...
const _sfc_main$1 = {
  ...
  setup(__props) {
    ...
    const stack = void 0;
    const _Error404 = vueExports.defineAsyncComponent(() => import('./error-404-C6Itz7WM.mjs'));
...
export { _export_sfc as _, useRoute as a, useSchemaOrg as b, useRouter as c, useNuxtApp as d, entry_default as default, useRuntimeConfig as e, nuxtLinkDefaults as f, navigateTo as n, resolveRouteObject as r, useHead as u };
